...
void Raytracer::createShaderBindingTable(
  ...
  assert(bufferSize > 0 && "SBT buffer size is zero. Check pipeline groups and SBT generator initialization.");

  // Create SBT buffer using the size from above
  NVVK_CHECK(m_allocator.createBuffer(
      m_sbtBuffer, bufferSize,
...
void Raytracer::raytrace(const VkCommandBuffer &cmd) {
  ...
  write.append(m_rtDescPack.makeWrite(0), m_gBuffers.getColorImageView(),
